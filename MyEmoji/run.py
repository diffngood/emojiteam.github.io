...
def align_and_crop_face(
    ...
    if output_size < transform_size:
        ...
        #img = img.resize((output_size, output_size), resample=Image.ANTIALIAS)
...
def run_projection(
    ...
    model = torch.hub.load("bryandlee/animegan2-pytorch:main", "generator", pretrained=mod, device=device)
    # 1: celeba_distill, 2: paprika, 3: face_paint_512_v1, 4: face_paint_512_v2

    img = Image.open(input_path).convert("RGB")

    painted_face = face2paint(model=model, img=img, size=512)
    painted_face.save(output_path)
...
#        painted_face = face2paint(model=model, img=face, size=512)
#        #display(painted_face)
#        painted_face.save(output_path)
